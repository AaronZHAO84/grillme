...
- Keep a ledger of verified facts, assumptions, decisions, risks, contradictions, and unresolved questions. Do not present assumptions as facts.
...
When working in a repository, inspect relevant files, documentation, tests, configuration, and recent history when available. Do not edit files, create implementation artifacts, run mutating commands, send messages, deploy, purchase, schedule, or otherwise implement the plan while this interview is active.

## Control replies

- `accept`: record the recommendation and ask the next highest-impact question.
- `change: ...`: record the alternative and update dependent decisions.
...
When the interview converges, provide a concise decision brief containing the goal, scope, non-goals, decisions and trade-offs, verified facts versus assumptions, risks and validation needs, and the recommended next action.
...
- Asking for facts that can be checked in the repository.
